...
async fn user_update_block() {
  ...
  assert!(client.update_block(new_block_number, new_block_timestamp,).is_ok());
...
async fn fork_into_arbiter() {
  ...
  let balance = weth.balance_of(address_to_check_balance).call().await.unwrap();
...
async fn middleware_from_forked_eo() {
  ...
  let vitalik_as_a_client = ArbiterMiddleware::new_from_forked_eoa(&environment, *vitalik_address);
  ...
  let weth = bindings::weth::WETH::deploy(vitalik_as_a_client.clone(), ()).unwrap().send().await;
  assert!(weth.is_ok()); // vitalik deployed the weth contract

  // test a non mutating call from the forked eoa
  let eth_balance = vitalik_as_a_client.get_balance(*vitalik_address, None).await.unwrap();
  assert_eq!(eth_balance, eU256::from(934034962177715175765_u128));
}
...
async fn block_logs() {
  ...
  let arbiter_token = deploy_arbx(client.clone()).await;
  arbiter_token.mint(Address::zero(), eU256::from(1000)).send().await.unwrap().await.unwrap();

  let new_block_number = 69;
  let new_block_timestamp = 420;

  client.update_block(new_block_number, new_block_timestamp).unwrap();

  arbiter_token.approve(Address::zero(), eU256::from(1000)).send().await.unwrap().await.unwrap();
  ...
  assert_eq!(logs.get(&revm::primitives::U256::from(69)).unwrap().len(), 1);
}
