...
//!
//! Core structures:
//! - [`Environment`]: Represents the Ethereum execution environment, allowing for its management
//!   (e.g., starting, stopping) and interfacing with agents.
...
///
/// ## Features
/// * [`revm::Evm`] and its connections to the "outside world" (agents) via the [`Socket`] provide
///   the [`Environment`] a means to route and execute transactions.
/// * [`ArbiterDB`] is the database structure used that allows for read-only sharing of execution
///   and write-only via the main thread. This can also be a database read in from disk storage via
///   [`database::fork::Fork`].
/// * [`ArbiterInspector`] is an that allows for the EVM to be able to display logs and properly
///   handle gas payments.
/// * [`EnvironmentParameters`] are used to set the gas limit, contract size limit, and label for
///   the [`Environment`].
#[derive(Debug)]
...
impl EnvironmentBuilder {
  ...
  /// [`EnvironmentBuilder`].
  pub fn build(self) -> Environment { Environment::create(self.parameters, self.db).run() }
  ...
  pub fn with_logs(mut self, logs: impl Into<std::collections::HashMap<U256, Vec<eLog>>>) -> Self {
...
impl Environment {
  ...
  fn create(parameters: EnvironmentParameters, db: ArbiterDB) -> Self {
    ...
    let inspector = if parameters.console_logs || parameters.pay_gas {
      Some(ArbiterInspector::new(parameters.console_logs, parameters.pay_gas))
    } else {
      Some(ArbiterInspector::new(false, false))
    };

    Self { socket, inspector, parameters, db, handle: None }
  }
  ...
  fn run(mut self) -> Self {
    ...
    let handle = thread::spawn(move || {
      ...
      while let Ok(instruction) = instruction_receiver.recv() {
        trace!("Instruction {:?} received by environment labeled: {:?}", instruction, label);
        match instruction {
          Instruction::AddAccount { address, outcome_sender } => {
          ...
          Instruction::BlockUpdate { block_number, block_timestamp, outcome_sender } => {
          ...
          Instruction::Cheatcode { cheatcode, outcome_sender } => match cheatcode {
            Cheatcodes::Load { account, key, block: _ } => {
              ...
              match db.state.write()?.accounts.get_mut(&recast_address) {
                Some(account) => {
                  ...
                  outcome_sender
                    .send(Ok(Outcome::CheatcodeReturn(CheatcodesReturn::Load { value })))?;
                },
                None => {
                  outcome_sender.send(Err(ArbiterCoreError::AccountDoesNotExistError))?;
                },
              };
            },
            Cheatcodes::Store { account, key, value } => {
              let recast_address = Address::from(account.as_fixed_bytes());
              ...
              match db.state.write()?.accounts.get_mut(&recast_address) {
                Some(account) => {
                  account.storage.insert(recast_key.into(), recast_value.into());

                  outcome_sender.send(Ok(Outcome::CheatcodeReturn(CheatcodesReturn::Store)))?;
                },
                None => {
                  outcome_sender.send(Err(ArbiterCoreError::AccountDoesNotExistError))?;
                },
            ...
            Cheatcodes::Deal { address, amount } => {
              ...
              match db.state.write()?.accounts.get_mut(&recast_address) {
                Some(account) => {
                  ...
                  outcome_sender.send(Ok(Outcome::CheatcodeReturn(CheatcodesReturn::Deal)))?;
                },
                None => {
                  outcome_sender.send(Err(ArbiterCoreError::AccountDoesNotExistError))?;
                },
              ...
              match db.state.write()?.accounts.get(&recast_address) {
                Some(account) => {
                  let account_state = match account.account_state {
                    ...
                    AccountState::StorageCleared => AccountStateSerializable::StorageCleared,
                    AccountState::NotExisting => AccountStateSerializable::NotExisting,
                  };
          ...
          // console logs.
          Instruction::Call { tx_env, outcome_sender } => {
            ...
            *evm.tx_mut() = tx_env;
            ...
            if let Some(console_log) = &mut evm.context.external.console_log {
              console_log.0.drain(..).for_each(|log| {
                ...
                trace!("Console logs: {:?}", HardhatConsoleCalls::decode(log).unwrap().to_string())
              });
            };

            outcome_sender.send(Ok(Outcome::CallCompleted(result)))?;
          },
          Instruction::SetGasPrice { gas_price, outcome_sender } => {
          ...
          Instruction::Transaction { tx_env, outcome_sender } => {
            // Set the tx_env and prepare to process it
            *evm.tx_mut() = tx_env;
            ...
            let receipt_data =
              ReceiptData { block_number, transaction_index, cumulative_gas_per_block };
            ...
            match logs.get_mut(&evm.block().number) {
              ...
              None => {
                logs.insert(
                  evm.block().number,
                  revm_logs_to_ethers_logs(execution_result.logs().to_vec(), &receipt_data),
                );
              },
            }

            match event_broadcaster
              .send(Broadcast::Event(execution_result.logs().to_vec(), receipt_data.clone()))
            {
              Ok(_) => {},
              Err(_) => {
            ...
            outcome_sender
              .send(Ok(Outcome::TransactionCompleted(execution_result, receipt_data)))?;

            transaction_index += U64::from(1);
          },
          Instruction::Query { environment_data, outcome_sender } => {
            let outcome = match environment_data {
              ...
              EnvironmentData::GasPrice => Ok(Outcome::QueryReturn(evm.tx().gas_price.to_string())),
              EnvironmentData::Balance(address) => {
                ...
                {
                  Some(account) => Ok(Outcome::QueryReturn(account.info.balance.to_string())),
              ...
              EnvironmentData::TransactionCount(address) => {
                match db
                  ...
                  .get::<Address>(&address.as_fixed_bytes().into())
                {
                  Some(account) => Ok(Outcome::QueryReturn(account.info.nonce.to_string())),
                  None => Err(ArbiterCoreError::AccountDoesNotExistError),
                }
              },
              EnvironmentData::Logs { filter } => {
                ...
                return_logs.retain(|log| {
                  filter.topics.iter().any(|topic_option| match topic_option {
                    Some(topic_val_or_array) => match topic_val_or_array {
                      ...
                      ValueOrArray::Array(topics) => topics.iter().any(|topic| match topic {
                        Some(topic) => log.topics.contains(topic),
                        None => true,
                      }),
                    },
                ...
                return_logs.retain(|log| {
                  filter.address.iter().any(|address_value_or_array| match address_value_or_array {
                ...
                Ok(Outcome::QueryReturn(serde_json::to_string(&return_logs).unwrap()))
              },
            };
  ...
  pub fn stop(mut self) -> Result<ArbiterDB, ArbiterCoreError> {
    ...
    self.socket.instruction_sender.send(Instruction::Stop(outcome_sender))?;
    ...
    self.handle.take().unwrap().join().map_err(|_| ArbiterCoreError::JoinError)??;
    Ok(db)
  }
}
...
mod tests {
  ...
  fn new_with_parameters() {
    ...
    assert_eq!(environment.parameters.contract_size_limit.unwrap(), TEST_CONTRACT_SIZE_LIMIT);
    assert_eq!(environment.parameters.gas_limit.unwrap(), U256::from(TEST_GAS_LIMIT));
  }
