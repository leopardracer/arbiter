...
use ethers::{
  ...
  types::{
    ...
    Address as eAddress, BlockId, Bloom, Bytes as eBytes, FilteredParams, NameOrAddress, Signature,
...
impl Signer for ArbiterMiddleware {
  ...
  fn with_chain_id<T: Into<u64>>(self, chain_id: T) -> Self {
    match self.wallet {
      EOA::Forked(_) => self,
      EOA::Wallet(wallet) => Self { wallet: EOA::Wallet(wallet.with_chain_id(chain_id)), ..self },
    }
  }
}
...
impl ArbiterMiddleware {
  ...
  pub fn new(
    ...
    connection.instruction_sender.upgrade().ok_or(ArbiterCoreError::UpgradeSenderError)?.send(
  ...
  pub fn new_from_forked_eoa(
    ...
    Ok(Arc::new(Self { wallet: EOA::Forked(forked_eoa), provider, label: None }))
  }
  ...
  pub fn update_block(
    ...
    provider.instruction_sender.upgrade().ok_or(ArbiterCoreError::UpgradeSenderError)?.send(
  ...
  pub async fn apply_cheatcode(
    ...
    let provider = self.provider.as_ref();
    provider.instruction_sender.upgrade().ok_or(ArbiterCoreError::UpgradeSenderError)?.send(
      Instruction::Cheatcode { cheatcode, outcome_sender: provider.outcome_sender.clone() },
    )?;
  ...
  pub async fn set_gas_price(
    ...
    let provider = self.provider.as_ref();
    provider.instruction_sender.upgrade().ok_or(ArbiterCoreError::UpgradeSenderError)?.send(
      Instruction::SetGasPrice { gas_price, outcome_sender: provider.outcome_sender.clone() },
    )?;
    match provider.outcome_receiver.recv()?? {
...
impl Middleware for ArbiterMiddleware {
  ...
  fn default_sender(&self) -> Option<eAddress> { Some(self.address()) }
  ...
  async fn send_transaction<T: Into<TypedTransaction> + Send + Sync>(
    ...
    let tx_env = TxEnv {
      ...
      data: revm_primitives::Bytes(bytes::Bytes::from(
        tx.data().ok_or(ArbiterCoreError::MissingDataError)?.to_vec(),
      )),
      chain_id: None,
      nonce: None,
      access_list: Vec::new(),
    ...
    if let Outcome::TransactionCompleted(execution_result, receipt_data) = outcome {
      match execution_result {
        ExecutionResult::Revert { gas_used, output } => {
          return Err(ArbiterCoreError::ExecutionRevert { gas_used, output: output.to_vec() });
        },
        ...
        ExecutionResult::Success { output, gas_used, logs, .. } => {
          ...
          match output {
            Output::Create(_, address) => {
              ...
              let state_ptr: *mut PendingTxState = &mut pending_tx as *mut _ as *mut PendingTxState;
  ...
  async fn call(
    ...
    let instruction =
      Instruction::Call { tx_env, outcome_sender: self.provider().as_ref().outcome_sender.clone() };
    self
      .provider()
      .as_ref()
      .instruction_sender
  ...
  async fn new_filter(&self, filter: FilterKind<'_>) -> Result<eU256, Self::Error> {
    ...
    let (_method, args) = match filter {
      FilterKind::NewBlocks =>
        unimplemented!("Filtering via new `FilterKind::NewBlocks` has not been implemented yet!"),
      FilterKind::PendingTransactions => {
        unimplemented!(
          "Filtering via `FilterKind::PendingTransactions` has not been implemented yet! 
                At the current development stage of Arbiter, transactions do not actually sit in a \
           pending state
    ...
    let filter_receiver = FilterReceiver { filter, receiver: Some(event_receiver) };
    provider.filter_receivers.lock().unwrap().insert(id, filter_receiver);
  ...
  async fn get_logs(&self, filter: &Filter) -> Result<Vec<eLog>, Self::Error> {
    ...
    provider.instruction_sender.upgrade().ok_or(ArbiterCoreError::UpgradeSenderError)?.send(
      Instruction::Query {
        environment_data: EnvironmentData::Logs { filter: filter.clone() },
        outcome_sender:   provider.outcome_sender.clone(),
      },
    )?;
    let outcome = provider.outcome_receiver.recv()??;
  ...
  async fn get_storage_at<T: Into<NameOrAddress> + Send + Sync>(
    ...
    let result =
      self.apply_cheatcode(Cheatcodes::Load { account: address, key, block }).await.unwrap();
...
#[inline]
pub fn recast_address(address: Address) -> eAddress { eAddress::from(address.into_array()) }
